...
def _build_args(x_in__, y_out_, p):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
...
@tbe_register.register_operator("Pdist", trans_bool_to_s8=False)
...
def pdist(x_in__, y_out_, p=2, kernel_name="pdist", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, p)
    ...
    origin_func_name = "pdist"
    ascendc_src_dir_ex = "pdist"
    ascendc_src_dir = "pdist"
    ascendc_src_file = "pdist.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator Pdist, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "Pdist"
...
def op_select_format(x_in__, y_out_, p=2, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, p)
    result = check_op_cap("op_select_format", "Pdist", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, p=2, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, p)
    result = check_op_cap("get_op_specific_info", "Pdist", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
